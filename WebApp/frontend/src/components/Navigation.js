 ...
 * @param {Function} toggleLayerVisibility - Function to toggle the visibility of a layer by its ID.
 * @param {Function} zoomToLayer - Function to zoom to a selected layer by its ID.
 * @param {Function} showAttributeTable - Function to display attribute table for the selected layer.
 * @param {Function} openAddWMSLayerModal - Function to open the modal for adding WMS/WMTS layers.
 * @param {Object} dragAndDropHandlers - Object containing drag-and-drop handler functions for reordering layers.
 * @param {Function} dragAndDropHandlers.handleDragStart - Function handling dragStart event, with event and index as parameters.
 * @param {Function} dragAndDropHandlers.handleDragOver - Function handling dragOver event.
 * @param {Function} dragAndDropHandlers.handleDrop - Function handling drop event, with event and index as parameters.
 */
const Navigation = ({
  layers,
  ...
  if (selectedLayer && selectedLayer.isVector) {
    ...
    setCurrentAttributes(attributes); // Ustaw dostępne kolumny na Dynamikę UI
  } else {
    console.error('Layer is not vector or invalid');
    setCurrentAttributes([]); // Usuń atrybuty, jeśli nie są dostępne
  }
};
